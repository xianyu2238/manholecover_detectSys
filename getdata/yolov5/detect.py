...
import sys
sys.path.append(r"D:\desktop\project\getdata\yolov5")
...
def detect(source, opt,save_img=True):
    # 各种传参数
...
def predict():
    ...
    try:
        ...
        # 3. 保存临时文件供YOLOv5检测使用（保持与你原有代码兼容）
        temp_input_path = "temp_input.jpg"
        cv2.imwrite(temp_input_path, img_array)


        opt=current_app.config['YOLO_OPT']
        # 4. 调用detect函数
        detected_img = detect(temp_input_path,opt)
